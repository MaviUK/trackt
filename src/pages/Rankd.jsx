...
function getFairPair(items, matchupMap, previousPairKey = "") {
  ...
  return Math.random() > 0.5 ? [chosen.first, chosen.second] : [chosen.second, chosen.first];
}

function getWinPercent(stats, showId) {
  const total = Number(stats?.times_matched || 0);
  if (!total) return 0;
  const wins = String(stats?.show_a_id) === String(showId)
...
function buildCommentTree(comments) {
  ...
  comments.forEach((comment) => byId.set(String(comment.id), { ...comment, replies: [] }));
...
function RankCard({ show, onChoose, onTouchStart, onTouchEnd }) {
  const recordText = `${show.rank_wins || 0}W-${show.rank_losses || 0}L • ${Math.round(show.rank_rating || DEFAULT_RATING)}`;
  return (
    <button
      type="button"
      onClick={onChoose}
    ...
    >
      ...
      <span className="rankd-card-record">{recordText}</span>
    </button>
  );
}
...
export default function Rankd() {
  ...
  const currentPairKey = currentPair.length === 2 ? makePairKey(currentPair[0].show_id, currentPair[1].show_id) : "";
    ...
    async function loadRankd() {
      try {
        ...
        const [watchedRows, allEpisodes, rankingRows, matchupRows] = await Promise.all([
          ...
          supabase.from("user_show_rankings").select("show_id, rating, wins, losses, comparisons").eq("user_id", user.id),
          supabase.from("rankd_matchups").select("*").or(
            showIds.map((id) => `show_a_id.eq.${id},show_b_id.eq.${id}`).join(",")
          ),
        ]);
        ...
        const watchedEpisodeIds = new Set((watchedRows || []).map((row) => row.episode_id).filter(Boolean).map(String));
        const rankingMap = new Map((rankingRows.data || []).map((row) => [String(row.show_id), row]));
        const nextMatchupMap = new Map((matchupRows.data || []).map((row) => [row.pair_key, row]));
        ...
        const withProgress = normalizedShows
          .map((show) => {
            ...
            const watchedMainCount = mainEpisodes.filter((ep) => watchedEpisodeIds.has(String(ep.id))).length;
        ...
        setLastPairKey(firstPair.length === 2 ? makePairKey(firstPair[0].show_id, firstPair[1].show_id) : "");
  ...
  useEffect(() => {
    ...
    const pair = showIds.map((id) => eligibleShows.find((show) => String(show.show_id) === String(id))).filter(Boolean);

    if (pair.length === 2) {
      setCurrentPair(pair);
      setLastPairKey(notification.rankd_matchups.pair_key);
      setShowComments(true);
      supabase.from("rankd_notifications").update({ is_read: true }).eq("id", notification.id).then(() => loadNotifications(userId));
    }
  }, [searchParams, notifications, eligibleShows, userId]);
  ...
  const leaderboard = useMemo(() => {
    return [...eligibleShows].sort((a, b) => {
      ...
      if ((b.rank_wins || 0) !== (a.rank_wins || 0)) return (b.rank_wins || 0) - (a.rank_wins || 0);
  ...
  async function handleChoice(winnerShowId) {
    ...
    const nextRatings = calculateNextRatings(winner.rank_rating || DEFAULT_RATING, loser.rank_rating || DEFAULT_RATING);
    ...
    try {
      ...
      const { data: recordedMatchup, error: matchupError } = await supabase.rpc("rankd_record_matchup_vote", {
        p_show_a_id: showAId,
        p_show_b_id: showBId,
      ...
      const nextMatchupRow = Array.isArray(recordedMatchup) ? recordedMatchup[0] : recordedMatchup;
      const updatedMatchupMap = new Map(matchupMap);
      if (nextMatchupRow?.pair_key) updatedMatchupMap.set(nextMatchupRow.pair_key, nextMatchupRow);
      setMatchupMap(updatedMatchupMap);

      const updatedShows = eligibleShows.map((show) => {
        if (String(show.show_id) === String(winner.show_id)) {
          return { ...show, rank_rating: nextRatings.winner, rank_wins: (show.rank_wins || 0) + 1, rank_comparisons: (show.rank_comparisons || 0) + 1 };
        }
        if (String(show.show_id) === String(loser.show_id)) {
          return { ...show, rank_rating: nextRatings.loser, rank_losses: (show.rank_losses || 0) + 1, rank_comparisons: (show.rank_comparisons || 0) + 1 };
        }
        return show;
      });
      ...
      setLastPairKey(nextPair.length === 2 ? makePairKey(nextPair[0].show_id, nextPair[1].show_id) : "");
      setCommentText("");
      setReplyTo(null);
  ...
  async function handleAddComment(event) {
    ...
    try {
      ...
      const { showAId, showBId } = getOrderedPair(currentPair[0].show_id, currentPair[1].show_id);
      const { data, error: commentError } = await supabase.rpc("rankd_add_matchup_comment", {
        p_show_a_id: showAId,
        p_show_b_id: showBId,
        p_body: body,
  ...
  function buildTouchStartHandler() {
    return (event) => { touchStartX.current = event.changedTouches?.[0]?.clientX ?? null; };
  }
    ...
    return (
      <div className="page rankd-page">
        <div className="page-shell">
          <div className="page-header"><h1>Rank'd</h1><p>Loading your matchups...</p></div>
        </div>
      </div>
    );
  }
  ...
  if (eligibleShows.length < 2) {
    return (
      <div className="page rankd-page">
        <div className="page-shell">
          <div className="page-header"><h1>Rank'd</h1><p>Swipe your favourite shows against each other to build your personal ranking.</p></div>
          <div className="section-card rankd-empty-card">
            ...
            <Link to="/my-shows" className="top-tab active">Go to My Shows</Link>
          </div>
        </div>
      </div>
    );
  ...
  const lastComment = [...comments].sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0];
  const currentTotalVotes = Number(matchupStats?.times_matched || 0);
    ...
    <div className="page rankd-page">
      <div className="page-shell">
        <div className="rankd-top-row">
          <div>
            <h1>Rank'd</h1>
            <p>Global matchup stats, comments, replies and fair rotation are now live.</p>
          </div>
          <button type="button" className="rankd-notification-button" onClick={() => setShowComments(true)}>
            Reply notifications {unreadCount ? <span>{unreadCount}</span> : null}
          </button>
        </div>

        <div className="rankd-feature-strip">
          <div><strong>Fair rotation</strong><span>Least-used show pairs are picked first</span></div>
          <div><strong>Global stats</strong><span>Every user vote updates this matchup</span></div>
          <div><strong>Comments</strong><span>Discuss, reply and get notified</span></div>
        </div>

        {notifications.length ? (
          <div className="section-card rankd-notifications-card">
            <div className="rankd-mini-heading">Latest replies</div>
            {notifications.slice(0, 3).map((notification) => (
              <button key={notification.id} type="button" onClick={() => openNotification(notification)} className={notification.is_read ? "rankd-notification-row" : "rankd-notification-row unread"}>
                <strong>{"Someone"} replied to your Rank'd comment</strong>
                <span>{notification.rankd_matchup_comments?.body}</span>
              </button>
            ))}
          </div>
        ) : null}

        {error ? <div className="section-card rankd-error-card"><strong>Something went wrong</strong><span>{error}</span></div> : null}

        <div className="rankd-main-grid">
          <div className="section-card rankd-battle-shell">
            <div className="rankd-battle-header">
              <div>
                <h2>{currentPair[0].show_name} vs {currentPair[1].show_name}</h2>
                <p>Click the show you prefer. This updates your ranking and the global matchup stats.</p>
              </div>
              <button type="button" className="rankd-skip-button" disabled={saving} onClick={() => { const nextPair = getFairPair(eligibleShows, matchupMap, currentPairKey); setCurrentPair(nextPair); setLastPairKey(nextPair.length === 2 ? makePairKey(nextPair[0].show_id, nextPair[1].show_id) : ""); }}>Skip</button>
            </div>
            <div className="rankd-battle-layout">
              <RankCard show={currentPair[0]} onChoose={() => handleChoice(currentPair[0].show_id)} onTouchStart={buildTouchStartHandler()} onTouchEnd={buildTouchEndHandler(currentPair[0].show_id, "left")} />
              <div className="rankd-battle-vs">VS</div>
              <RankCard show={currentPair[1]} onChoose={() => handleChoice(currentPair[1].show_id)} onTouchStart={buildTouchStartHandler()} onTouchEnd={buildTouchEndHandler(currentPair[1].show_id, "right")} />
            </div>

            <div className="rankd-matchup-dashboard">
              <div className="rankd-matchup-count">
                <strong>{currentTotalVotes}</strong>
                <span>global matches recorded for this exact pair</span>
              </div>
              <div className="rankd-win-bars">
                <div className="rankd-win-row">
                  <span>{currentPair[0].show_name}</span>
                  <div><i style={{ width: `${leftWinPercent}%` }} /></div>
                  <strong>{leftWinPercent}%</strong>
                </div>
                <div className="rankd-win-row">
                  <span>{currentPair[1].show_name}</span>
                  <div><i style={{ width: `${rightWinPercent}%` }} /></div>
            ...
            <form className="rankd-comment-form" onSubmit={handleAddComment}>
              {lastComment ? (
                <div className="rankd-last-comment">
                  <strong>Last comment</strong>
                  <p>{lastComment.body}</p>
                  <button type="button" className="rankd-text-button" onClick={() => setShowComments(true)}>View all comments</button>
                </div>
              ) : null}
              {replyTo ? (
              ...
              <div className="rankd-comment-actions">
                <button type="button" className="rankd-outline-button" onClick={() => setShowComments((value) => !value)}>{showComments ? "Hide all comments" : `View all comments (${comments.length})`}</button>
              </div>
              <textarea value={commentText} onChange={(event) => setCommentText(event.target.value)} placeholder="Write a brief comment about this matchup..." rows={3} />
              <button type="submit" disabled={saving || !commentText.trim()}>{replyTo ? "Post reply" : "Post comment"}</button>
            </form>
            ...
            {showComments ? (
              <div className="rankd-comments-panel">
                <div className="rankd-comments-header">
                  ...
                  <button type="button" className="rankd-text-button" onClick={() => setShowComments(false)}>Hide</button>
                </div>
                {commentTree.length ? commentTree.map((comment) => (
                  <CommentItem key={comment.id} comment={comment} currentUserId={userId} onReply={(selected) => { setReplyTo(selected); setCommentText(""); }} />
          ...
          <div className="section-card rankd-leaderboard-card">
            <div className="rankd-leaderboard-header">
              <div><h2>Your current order</h2><p>Live ranking based on every Rank'd decision.</p></div>
              <div>{leaderboard.length} shows</div>
            </div>

            <div className="rankd-leaderboard-list">
              {leaderboard.map((show, index) => (
                <Link key={show.show_id} to={`/my-shows/${show.tvdb_id}`} className="rankd-leaderboard-row">
                  <div className="rankd-leaderboard-poster">
                    {show.poster_url ? <img src={show.poster_url} alt={show.show_name} /> : <span>#{index + 1}</span>}
                  </div>
                  <div>
                    <div className="rankd-leaderboard-title">#{index + 1} {show.show_name}</div>
                    <div className="rankd-leaderboard-meta">{show.watchedMainCount} watched • {show.rank_wins || 0} wins • {show.rank_losses || 0} losses</div>
                  </div>
                  <div className="rankd-rating-pill">{Math.round(show.rank_rating || DEFAULT_RATING)}</div>
                </Link>
              ))}
            </div>
          </div>
